...
import { PlusSquareIcon, Loader2, CheckCircle, CornerDownLeft, PackageSearchIcon } from 'lucide-react';
import { db } from '@/lib/firebase';
import { collection, getDocs, query, orderBy, addDoc, serverTimestamp, doc } from 'firebase/firestore';
import type { InventoryItem, RequestItemWrite } from '@/types';
...
export default function NewRequestPage() {
  ...
  const onSubmit: SubmitHandler<NewRequestFormValues> = async (data) => {
    ...
    const selectedItem = inventoryList.find(item => item.id === data.itemId);
    if (!selectedItem) {
    ...
    const requestDataForFirestore: RequestItemWrite = {
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
              <CardHeader>
                ...
                <CardDescription>Select an item and specify the quantity needed.</CardDescription>
              </CardHeader>
